...
const Navbar = ({ onOpenModal }) => {
  ...
  return (
    <>
      <header
        className={`fixed${sticky ? ' sticky' : ''}`}
        style={{
          background: sticky
            ? 'rgba(5,5,16,0.88)'
            : 'transparent',
          backdropFilter: sticky ? 'blur(20px)' : 'none',
          WebkitBackdropFilter: sticky ? 'blur(20px)' : 'none',
          borderBottom: sticky ? '1px solid rgba(37,99,235,0.15)' : 'none',
          boxShadow: sticky ? '0 4px 30px rgba(0,0,0,0.4)' : 'none',
        ...
        <div className="main-header">
          <nav className="navbar navbar-expand-lg">
            <div className="container">
              ...
              >
                <span style={{ display: 'block', width: '22px', height: '2px', background: '#3b82f6', marginBottom: '5px' }} />
                <span style={{ display: 'block', width: '22px', height: '2px', background: '#3b82f6', marginBottom: '5px' }} />
                <span style={{ display: 'block', width: '22px', height: '2px', background: '#3b82f6' }} />
              </button>

              <div className={`collapse navbar-collapse${mobileOpen ? ' show' : ''}`}
                style={{ background: mobileOpen ? 'rgba(5,5,16,0.97)' : 'transparent', borderRadius: mobileOpen ? '12px' : '0', padding: mobileOpen ? '16px' : '0' }}
              >
                <div className="mainnavs">
                  ...
                  <div className="calltopdv" onClick={onOpenModal} style={{ cursor: 'pointer' }}>
                    ...
                    <div className="ct-info-holder">
                      ...
                      <a href="tel:+15104769126" onClick={(e) => e.stopPropagation()} />
                    </div>
                  </div>
                </div>
              </div>
      ...
      {/* Custom active-link glow style */}
      <style>{`
        .main-header nav .mainnavs a.active {
